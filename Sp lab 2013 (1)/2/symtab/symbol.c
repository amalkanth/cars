...
else   // if not alphabet take string upto a nonalphabet
{
  a[i]='\0';                   //a contains a string which may be a datatype
  i=0;
 if((strcmp(a,"int")==0)||(strcmp(a,"char")==0)||(strcmp(a,"float")==0))
 {
   ...
   while(isalpha(ch)!=0)
    {
     b[j]=ch;                 //b may be variable or function
     j++;
     ch=getc(fp);
    }
